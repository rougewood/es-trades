...
#     state = data.iloc[0:input_size, ].T
...
#         next_state = data.iloc[t:t + input_size, ].T
...
#             total_profit += data.iloc[t + input_size - 1, 0] - bought_price
#             tradeLog.write("Sell: " + formatPrice(data.iloc[t + input_size - 1, 0]) + " | Profit: " + formatPrice(
#                 data.iloc[t + input_size - 1, 0] - bought_price) + ", date : "
#                            + str(data.index[t + input_size - 1]))
#
#             print("Sell: " + formatPrice(data.iloc[t + input_size - 1, 0]) + " | Profit: "
#                   + formatPrice(data.iloc[t + input_size - 1, 0] - bought_price))
#
...
# plt.plot(np.linspace(1, episode_count, num=episode_count), epoch_profits, linestyle='dashed', color='blue')
plt.plot(1, 2, linestyle='dashed', color='blue')
plt.xlabel('epoch')
plt.ylabel('profit')
plt.show()
